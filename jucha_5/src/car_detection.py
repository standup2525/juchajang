...
class CarDetector:
    def __init__(self):
        # 현재 PyTorch YOLOv8은 AI Hat(NPU)을 직접 활용하지 않음
        # OpenVINO 기반 추론을 원한다면 별도 코드가 필요함
    ...
    def detect_cars(self, frame):
        ...
        for result in results:
            boxes = result.boxes
            for box in boxes:
                if int(box.cls) in self.car_class_ids:
...
class ParkingSystem:
    def __init__(self):
        ...
        self.parked_vehicles = {}  # Store parked vehicles
        self.total_spots = 10
        self.available_spots = list(range(1, self.total_spots + 1))
        
    def process_frame(self, frame):
        car_boxes = self.car_detector.detect_cars(frame)
        logger.info(f"[Info] Detected {len(car_boxes)} vehicles in frame.")
        detected_plates = []
        
        for (x1, y1, x2, y2) in car_boxes:
            logger.info(f"[Info] Vehicle box: ({x1},{y1},{x2},{y2})")
            car_roi = frame[y1:y2, x1:x2]
            plate_infos = self.plate_recognizer.find_license_plate(car_roi)
            logger.info(f"[Info]  - Found {len(plate_infos)} plate candidates in vehicle region.")
            ...
            for plate_info in plate_infos:
                ...
                # Make sure plate region is within image bounds
                ...
                logger.info(f"[Info]    - Plate candidate text: '{plate_text}'")
