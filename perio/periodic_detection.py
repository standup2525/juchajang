...
CAMERA_WIDTH = 640                  # Camera image width in pixels
CAMERA_HEIGHT = 360                 # Camera image height in pixels
    ...
    def process_detection_cycle(self):
        ...
        try:
            ...
            for bbox in vehicles:
                ...
                # Recognize plate
                plate_number = self.plate_recognizer.recognize_plate(vehicle_image)
                if not plate_number:
                    continue
                
                # Check for duplicate detection
                if is_duplicate_detection(plate_number):
                    continue
                ...
                # Save image
                image_path = save_vehicle_image(vehicle_image, plate_number, direction)
                
                # Send to server
                timestamp = datetime.now().isoformat()
                self.server_communicator.send_vehicle_data(
                    plate_number, direction, timestamp,
    ...
    def run(self):
        ...
        try:
            while self.system_running:
                ...
                wait_time = max(0, DETECTION_INTERVAL - elapsed)
